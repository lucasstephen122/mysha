	...
	class Mongodb
	{
		...
		public function connect($config , $tries = 6)
		{
			try
			{
				...
				$connection = new $MongoClient("mongodb://".$config['host'].":".$config['port'] , array('connectTimeoutMS' => 1000));
		...
		{
			$instance_key = $config['host'].':'.$config['port'].':'.$config['database'];
		...
		public static function clone_collection($from_config , $from_instance , $from_collection , $to_config , $to_instance , $to_collection , $criteria = array() , $fields = array())
		{
			...
			if($from_config['host'] == $to_config['host'])
			{	
				$query = array();
		...
		public function get($table_name , $conditions = array() , $order = array() , $limit = array())
		{
			...
			if(empty($row))
			{
				throw new NoRecordException(NoRecordException::$EX_CODE_NOT_FOUND, $conditions);
			}
			return $row;
		}
		...
		public function get_row()
		{
			try 
			{
				...
				{
					throw new NoRecordException(NoRecordException::$EX_CODE_NOT_FOUND, array());
				}
			}
		...
		public function update($table_name , $record , $conditions = array() , $order = array() , $limit = array())
		{
			try 
			{
				...
				if($error['n'] == 0)
				{
					...
					throw new NoRecordException(NoRecordException::$EX_CODE_NO_UPDATE, $conditions);
				}
		...
		public function delete($table_name , $conditions = array() , $order = array() , $limit = array())
		{
			try 
			{
				...
				if($error['n'] == 0)
				{
					throw new NoRecordException(NoRecordException::$EX_CODE_NO_UPDATE, $mongo_conditions);
				}
				return $result;
			}
			catch (NoRecordException $e)
		...
		private function prepare_condition($column , $value)
		{
			...
			else 
			{
				...
				$operator_map = array('IS' => '', 'GT' => '$gt' , 'GTE' => '$gte' , 'IN' => '$in' , 'LT' => '$lt' , 'LTE' => '$lte' , 'NE' => '$ne' , 'NIN' => '$nin' , 'NOT' => '$ne' , 'LIKE' => '$regex' , 'BETWEEN' => '');
